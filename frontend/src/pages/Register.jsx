...
function Register() {
  ...
  useEffect(() => {
    ...
    tl.fromTo(containerRef.current, { opacity: 0 }, { opacity: 1, duration: 0.45 })
      .fromTo(cardRef.current, { y: 56, opacity: 0, scale: 0.95 }, { y: 0, opacity: 1, scale: 1, duration: 0.75, ease: 'power3.out' }, '-=0.05')
      .fromTo(titleRef.current, { y: 14, opacity: 0 }, { y: 0, opacity: 1, duration: 0.5 }, '-=0.35')
      .fromTo(inputRefs.current, { x: -24, opacity: 0 }, { x: 0, opacity: 1, duration: 0.45, stagger: 0.1 }, '-=0.2')
      .fromTo(buttonRef.current, { y: 12, opacity: 0 }, { y: 0, opacity: 1, duration: 0.4 }, '-=0.18');
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate('/verify-email', { state: { email: formData.email, devOtp: data.devOtp || '' } });
    ...
    <section ref={containerRef} className="mx-auto flex min-h-screen w-full max-w-7xl items-center justify-center px-4 py-10">
      <AuthCard cardRef={cardRef}>
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          >
            {loading ? <Loader label="Creating account" /> : 'Register'}
          </button>

          <p className="pt-2 text-center text-sm text-slate-300">
            Already have an account?{' '}
            <Link to="/login" onClick={animateAndNavigate('/login')} className="font-semibold text-cyan-300 hover:text-cyan-200">
              Login
            </Link>
          </p>
        </form>
